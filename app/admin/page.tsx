...
export default function AdminDashboard() {
  ...
  const handleUpdateReviewStatus = async (id: string, status: Review['status']) => {
    try {
      await SupabaseStore.updateReview(id, { status })
      await loadData()
  ...
  const handleSubmitReviewResponse = async () => {
    ...
    try {
      await SupabaseStore.updateReview(respondingReview.id, {
  ...
  const handleDeleteReview = async (id: string) => {
    if (confirm('Êtes-vous sûr de vouloir supprimer cet avis ?')) {
      try {
        await SupabaseStore.deleteReview(id)
        await loadData()
  ...
  const handleChangePassword = async () => {
    ...
    const result = await authChangePassword(session.user.id, currentPassword, newPassword)
  ...
  const handleChangeEmail = async () => {
    ...
    // Changer l'email via Supabase
    const result = await authChangeEmail(session.user.id, newAdminEmail)
  ...
  const handleSaveAgencyInfo = async () => {
    try {
      await SupabaseStore.updateAgencySettings(agencyConfig)
  ...
  const handleAddPartner = async (partner: Omit<Partner, 'id' | 'createdAt' | 'updatedAt'>) => {
    try {
      ...
      await SupabaseStore.addPartner(newPartner)
      await loadData()
  ...
  const handleUpdatePartner = async (id: string, updates: Partial<Partner>) => {
    try {
      await SupabaseStore.updatePartner(id, updates)
      await loadData()
      setEditingPartner(null)
  ...
  const handleDeletePartner = async (id: string) => {
    if (confirm('Êtes-vous sûr de vouloir supprimer ce partenaire ?')) {
      try {
        await SupabaseStore.deletePartner(id)
        await loadData()
  ...
  const handleTogglePartner = async (id: string, actif: boolean) => {
    try {
      await SupabaseStore.updatePartner(id, { actif: !actif })
      await loadData()
  ...
  const handleMovePartner = async (id: string, direction: 'up' | 'down') => {
    ...
    try {
      await SupabaseStore.updatePartner(partners[currentIndex].id, { ordre: targetOrdre })
      await SupabaseStore.updatePartner(partners[newIndex].id, { ordre: currentOrdre })
      await loadData()
    } catch (error) {
  ...
  const handleAddFaq = async (faq: Omit<FAQ, 'id' | 'createdAt' | 'updatedAt'>) => {
    try {
      ...
      await SupabaseStore.addFAQ(newFaq)
      await loadData()
  ...
  const handleUpdateFaq = async (id: string, updates: Partial<FAQ>) => {
    try {
      await SupabaseStore.updateFAQ(id, updates)
      await loadData()
      setEditingFaq(null)
      setShowFaqForm(false)
  ...
  const handleDeleteFaq = async (id: string) => {
    if (confirm('Êtes-vous sûr de vouloir supprimer cette FAQ ?')) {
      try {
        await SupabaseStore.deleteFAQ(id)
        await loadData()
  ...
  const handleToggleFaq = async (id: string, actif: boolean) => {
    try {
      await SupabaseStore.updateFAQ(id, { actif: !actif })
      await loadData()
  ...
  const handleMoveFaq = async (id: string, direction: 'up' | 'down') => {
    ...
    try {
      await SupabaseStore.updateFAQ(faqs[currentIndex].id, { ordre: targetOrdre })
      await SupabaseStore.updateFAQ(faqs[newIndex].id, { ordre: currentOrdre })
      await loadData()
    } catch (error) {
      toast.error('Erreur lors du déplacement')
    }
  ...
  const handleAddBlogPost = async (data: Omit<BlogPost, 'id' | 'createdAt' | 'updatedAt' | 'views'>) => {
    try {
      await SupabaseStore.addBlogPost(data)
      await loadData()
  ...
  const handleUpdateBlogPost = async (data: Omit<BlogPost, 'id' | 'createdAt' | 'updatedAt' | 'views'>) => {
    ...
    try {
      await SupabaseStore.updateBlogPost(editingBlogPost.id, data)
      await loadData()
  ...
  const handleAddProperty = async (data: PropertyFormData) => {
    try {
      ...
      await SupabaseStore.addProperty(newProperty)
      await loadData()
  ...
  const handleUpdateProperty = async (data: PropertyFormData) => {
    ...
    try {
      ...
      await SupabaseStore.updateProperty(editingProperty.id, updatedProperty)
      await loadData()
      setShowPropertyForm(false)
      setEditingProperty(null)
      setEditingFormData(undefined)
                        ...
                        <div className="flex items-center justify-end space-x-2">
                          ...
                          <button 
                            onClick={async () => {
                              ...
                              await SupabaseStore.updateProperty(property.id, { status: newStatus })
                  ...
                  <div className="flex gap-2 pt-3 border-t border-gray-200">
                    ...
                    <button
                      onClick={async (e) => {
                        e.stopPropagation()
                        try {
                          await SupabaseStore.updateMessage(message.id, { status: 'repondu' })
                          await loadData()
                    ...
                    <tr key={post.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-4">
                        <button
                          onClick={async () => {
                            try {
                              await SupabaseStore.updateBlogPost(post.id, { isPublished: !post.isPublished })
                              await loadData()
                      ...
                      <td className="px-4 py-4">
                        <div className="flex items-center gap-2">
                          <button
                            onClick={async () => {
                              try {
                                await SupabaseStore.toggleBlogPostPin(post.id)
                                await loadData()
                          ...
                          <button
                            onClick={async () => {
                              if (confirm('Supprimer cet article ?')) {
                                try {
                                  await SupabaseStore.deleteBlogPost(post.id)
                                  await loadData()
                    ...
                    <tr key={request.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3">
                        <select
                          ...
                          onChange={async (e) => {
                            try {
                              await SupabaseStore.updateClientRequest(request.id, { status: e.target.value as any })
                              await loadData()
                        ...
                        <button
                          onClick={async () => {
                            if (confirm('Supprimer cette demande ?')) {
                              try {
                                await SupabaseStore.deleteClientRequest(request.id)
                                await loadData()
