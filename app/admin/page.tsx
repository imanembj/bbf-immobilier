...
// Le store local gère localStorage en dev et MySQL en production via API routes
...
export default function AdminDashboard() {
  ...
  const handleSubmitReviewResponse = async () => {
    ...
    try {
      const store = getStore()
      store.updateReview(respondingReview.id, {
  ...
  const handleUpdatePartner = async (id: string, updates: Partial<Partner>) => {
    try {
      const store = getStore()
      store.updatePartner(id, updates)
      await loadData()
      setEditingPartner(null)
  ...
  const handleTogglePartner = async (id: string, actif: boolean) => {
    try {
      const store = getStore()
      store.updatePartner(id, { actif: !actif })
      await loadData()
  ...
  const handleMovePartner = async (id: string, direction: 'up' | 'down') => {
    ...
    try {
      const store = getStore()
      store.updatePartner(partners[currentIndex].id, { ordre: targetOrdre })
      store.updatePartner(partners[newIndex].id, { ordre: currentOrdre })
      await loadData()
    } catch (error) {
  ...
  const handleToggleFaq = async (id: string, actif: boolean) => {
    try {
      const store = getStore()
      store.updateFAQ(id, { actif: !actif })
      await loadData()
  ...
  const handleMoveFaq = async (id: string, direction: 'up' | 'down') => {
    ...
    try {
      const store = getStore()
      store.updateFAQ(faqs[currentIndex].id, { ordre: targetOrdre })
      store.updateFAQ(faqs[newIndex].id, { ordre: currentOrdre })
      await loadData()
    } catch (error) {
      toast.error('Erreur lors du déplacement')
    }
  ...
  const handleUpdateBlogPost = async (data: Omit<BlogPost, 'id' | 'createdAt' | 'updatedAt' | 'views'>) => {
    ...
    try {
      const store = getStore()
      store.updateBlogPost(editingBlogPost.id, data)
      await loadData()
  ...
  const handleUpdateProperty = async (data: PropertyFormData) => {
    ...
    try {
      ...
      const store = getStore()
      store.updateProperty(editingProperty.id, updatedProperty)
      await loadData()
      setShowPropertyForm(false)
      setEditingProperty(null)
      setEditingFormData(undefined)
                        ...
                        <div className="flex items-center justify-end space-x-2">
                          ...
                          <button 
                            onClick={async () => {
                              ...
                              const store = getStore()
                              store.updateProperty(property.id, { status: newStatus })
                  ...
                  <div className="flex gap-2 pt-3 border-t border-gray-200">
                    ...
                    <button
                      onClick={async (e) => {
                        ...
                        try {
                          const store = getStore()
                          store.updateMessage(message.id, { status: 'repondu' })
                          await loadData()
                      ...
                      <td className="px-4 py-4">
                        <button
                          onClick={async () => {
                            try {
                              const store = getStore()
                              store.updateBlogPost(post.id, { isPublished: !post.isPublished })
                              await loadData()
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            onClick={async () => {
                              try {
                                const store = getStore()
                                store.toggleBlogPostPin(post.id)
                                await loadData()
                      ...
                      <td className="px-4 py-3">
                        <select
                          ...
                          onChange={async (e) => {
                            try {
                              const store = getStore()
                              store.updateClientRequest(request.id, { status: e.target.value as any })
                              await loadData()
                        ...
                        <button
                          onClick={async () => {
                            if (confirm('Supprimer cette demande ?')) {
                              try {
                                const store = getStore()
                                store.deleteClientRequest(request.id)
                                await loadData()
